...
def fetch_usda_foods(query: str, page_size: int = 10, page_number: int = 1) -> list[Food]:
    """Search USDA by food name, return a list of normalized Food objects.
    Uses USDA's natural relevance ranking across all databases, supporting
    pagination via page_number and page_size.
...
def fetch_off_product(barcode: str) -> Food | None:
    ...
    params = {"fields": "product_name,nutriments,serving_quantity,nova_group,categories_tags"}
